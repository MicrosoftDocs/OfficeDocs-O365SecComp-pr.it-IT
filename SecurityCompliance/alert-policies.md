...
Le autorizzazioni di controllo di accesso basi ruoli (RBAC) assegnate agli utenti dell'organizzazione determina quale degli avvisi possono essere visualizzate nella pagina **Visualizza avvisi** . Come eseguire questa operazione? I ruoli di gestione assegnati agli utenti (in base all'appartenenza a gruppi di ruoli in & la sicurezza centro conformità) determinano quali categorie di avvisi possono essere visualizzate nella pagina di **visualizzare gli avvisi** . Di seguito sono riportati alcuni esempi:
...
Per verificare che la categoria a cui è assegnato un criterio di avviso predefinito per, vedere la tabella nella sezione [criteri di avviso predefinito](#default-alert-policies) .
