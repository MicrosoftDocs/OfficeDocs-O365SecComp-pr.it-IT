...
|Da  <br/> |Il mittente di un messaggio di posta elettronica.<sup>1</sup> <br/> |`from:pilarp@contoso.com`  <br/> `from:contoso.com`  <br/> |I messaggi inviati dall'utente specificato o da un dominio specificato.  <br/> |
...
|A  <br/> |Il campo To (A) di un messaggio di posta elettronica.<sup>1</sup> <br/> |`to:annb@contoso.com`  <br/> `to:annb ` <br/> `to:"Ann Beebe"`  <br/> |Tutti gli esempi restituiscono i messaggi in cui è specificato l'utente Ann Beebe nella riga To: (A:).  <br/> |
...
> [!TIP]
> Per cercare valori che contengono spazi o caratteri speciali, utilizzare le virgolette doppie ("") per contengono la frase; ad esempio `businessaddress:"123 Main Street"`. 
...
|ONEAR  <br/> |keyword1 ONEAR(n) keyword2  <br/> |Allo stesso **quasi**, ma restituisce gli elementi con le parole scritte vicini nell'ordine specificato. Ad esempio `best ONEAR(5) worst` restituisce qualsiasi elemento in cui la parola "ottimale" si verifica prima della parola "peggiore" e le parole sono all'interno di cinque parole tra loro. Se viene specificato alcun numero, la distanza predefinita è otto parole. <sup>2</sup> <br/> > [!NOTE]non è supportata l'operatore **ONEAR** durante la ricerca di cassette postali, gt _ funziona solo per la ricerca di SharePoint e OneDrive i siti. Se si esegue la ricerca cassette postali e i siti nella stessa ricerca e la query che include l'operatore **ONEAR** , la ricerca verrà restituiti gli elementi della cassetta postale come se si utilizza l'operatore **NEAR** . In altre parole, la ricerca restituisce gli elementi in cui le parole specificate sono vicini indipendentemente dall'ordine in cui vengono eseguite le parole.           |
...
|A  <br/> |Il destinatario del messaggio di posta elettronica.  <br/> |
...
|Creato  <br/> |La data di creazione di un documento.  <br/> |
|Data ultima modifica  <br/> |La data dell'ultima modifica di un documento.  <br/> |
...
Mentre il `ViewableByExternalUsers` proprietà rappresenta lo stato dei se un documento o un sito è condivisa con utenti esterni, esistono alcuni avvisi a quanto questa proprietà è e non riflette. Negli scenari seguenti, il valore del `ViewableByExternalUsers` proprietà non vengono aggiornati e potrebbero non essere precisi i risultati di una query di ricerca del contenuto che utilizza questa proprietà. 
...
![Verranno restituiti solo i file condivisi con specifici utenti da una query di ricerca che utilizza la proprietà SharedWithUsersOWSUser](media/469a4b61-68bd-4ab0-b612-ab6302973886.png)
...
## <a name="searching-for-skype-for-business-conversations"></a>Ricerca di Skype per le conversazioni aziendali

Eseguire la ricerca in modo specifico per il contenuto in Skype per le conversazioni di Business, è possibile utilizzare le query con parole chiave seguenti:

```
kind:im
```

Si noti che la query di ricerca precedente anche restituisce le chat da Microsoft Teams. Per evitare questo problema, è possibile limitare i risultati della ricerca in modo da includere solo Skype per le conversazioni Business utilizzando le query con parole chiave seguenti:

```
kind:im AND subject:conversation
```

La query con parole chiave precedente consente di escludere le chat di Microsoft Teams perché Skype per le conversazioni aziendali vengono salvati come messaggi di posta elettronica con riga dell'oggetto che inizia con la parola "Conversazione".

Per cercare Skype per le conversazioni Business che si sono verificati all'interno di un intervallo di date specifiche, utilizzare la query con parole chiave seguenti:
...
- È possibile esportare gli elementi in base al tipo di messaggio. Ad esempio, per esportare Skype conversazioni e le chat di Microsoft Teams, utilizzare la sintassi `kind:im`. Per restituire solo i messaggi di posta elettronica, è necessario utilizzare `kind:email`. Per restituire le chat, riunioni e le chiamate all'interno di Microsoft Teams, utilizzare `kind:microsoftteams`.
