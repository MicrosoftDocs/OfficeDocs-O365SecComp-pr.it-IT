...
2. &amp; Nel riquadro sinistro del Centro sicurezza e conformità fare clic su **importazione**di governance **** \> dei dati.
    ...
    > [!NOTE]
    > È necessario disporre delle autorizzazioni appropriate per accedere alla pagina di **importazione** nel centro sicurezza &amp; e conformità. Per ulteriori informazioni, vedere la sezione **prima di iniziare** . 
...
6. Nella pagina **Importa dati** eseguire le due operazioni seguenti: 
    ...
    a. nel passaggio 2, fare clic su **Mostra URL SAS caricamento di rete**. Dopo aver visualizzato l'URL SAS, fare clic su **copia negli Appunti** e quindi incollarlo e salvarlo in un file in modo che sia possibile accedervi in un secondo momento.
    
    b. nel passaggio 3, fare clic su **download di Azure AzCopy** per scaricare e installare lo strumento di AzCopy di Azure. Nella finestra popup, fare clic su **Esegui** per installare AzCopy. 
...
3. Eseguire il seguente comando per caricare i file PST in Office 365.
    ...
    Nella tabella seguente vengono descritti i parametri e i valori necessari. Si noti che le informazioni ottenute nel passaggio precedente vengono utilizzate nei valori di questi parametri.
...
> [!TIP]
> Dopo aver eseguito correttamente il comando AzCopy. exe e aver verificato che tutti i parametri siano corretti, salvare una copia della sintassi della riga di comando nello stesso file (protetto) in cui sono state copiate le informazioni ottenute nel passaggio 1. È quindi possibile copiare e incollare il comando in un prompt dei comandi ogni volta che si desidera eseguire lo strumento AzCopy. exe per caricare i file PST in Office 365. L'unico valore che potrebbe essere necessario modificare sono quelli per il `/Source:` parametro. Questo dipende dalla directory di origine in cui si trovano i file PST.
...
Come passaggio facoltativo, è possibile installare e utilizzare Microsoft Azure Storage Explorer (che è uno strumento gratuito open source) per visualizzare l'elenco dei file PST caricati nel BLOB di Azure. Per eseguire questa operazione, sono disponibili due buoni motivi:
...
6. Al termine dell'utilizzo di Microsoft Azure Storage Explorer, fare clic con il pulsante destro del mouse su **** **ingestiondata**e quindi scegliere Disconnetti per disconnettersi dall'area di archiviazione di Azure. In caso contrario, verrà visualizzato un messaggio di errore la volta successiva che si tenterà di allegare. 
...
3. Nella pagina **Importa** , fare clic ![su Aggiungi](media/ITPro-EAC-AddIcon.gif) **nuovo processo di importazione**.
    ...
    **Nota:** Per creare un nuovo processo di importazione, è necessario disporre **** delle autorizzazioni appropriate per accedere &amp; alla pagina Importa nel centro sicurezza e conformità. Per ulteriori informazioni, vedere la sezione **prima di iniziare** . 
...
1. Nella pagina **Importa** nel centro sicurezza &amp; e conformità fare clic su **pronto per l'importazione in Office 365** per il processo di importazione creato nel passaggio 5. 
...
- Perché importare i file PST in Office 365?
  ...
  - Aiuta a soddisfare le esigenze di conformità dell'organizzazione, consentendo di applicare le funzionalità di conformità di Office 365 ai dati dei file PST importati. Sono inclusi i seguenti:
