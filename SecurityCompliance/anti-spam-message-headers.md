...
|:-----|:-----|
|CIP: [indirizzo IP]|L'indirizzo IP di connessione. Potrebbe essere necessario specificare questo indirizzo IP quando si crea un elenco indirizzi IP conSentiti o un elenco indirizzi IP bloccati nel filtro di connessione. Per ulteriori informazioni, vedere [Configure the Connection Filter Policy](configure-the-connection-filter-policy.md).|
...
|dkim|Descrive i risultati del controllo DKIM per il messaggio. Tra i possibili valori sono inclusi:  <br/>• **pass**: indica che il controllo DKIM per il messaggio è stato superato. <br/>• **Fail (Reason)**: indica che il controllo DKIM per il messaggio non è riuscito e perché. Ad esempio, se il messaggio non è stato firmato o se la firma non è stata verificata.<br/>• **None**: indica che il messaggio non è stato firmato. Questo può o non può indicare che il dominio ha un record di DKIM o che il record DKIM non restituisce un risultato, solo che questo messaggio non è stato firmato.|
|header. d|Dominio identificato nella firma DKIM, se presente. Questo è il dominio sottoposto a query per la chiave pubblica.|
|dmarc|Descrive i risultati del controllo DMARC per il messaggio. Tra i possibili valori sono inclusi:  <br/>• **pass**: indica che il controllo DMARC per il messaggio è stato superato. <br/>• **Fail**: indica che il controllo DMARC per il messaggio non è riuscito. <br/>• **bestguesspass**: indica che non esiste alcun record TXT DMARC per il dominio, ma, se ne esiste uno, il controllo di DMARC del messaggio verrebbe superato. Ciò è dovuto al fatto che il dominio nell'indirizzo 5321. MailFrom corrisponde al dominio dell'indirizzo 5322. from.<br/>• **None**: indica che non esiste alcun record TXT DKIM per il dominio di invio in DNS.|
|action|Indica l'azione eseguita dal filtro posta indesiderata in base ai risultati del controllo DMARC. Ad esempio:  <br/>• **PermError**: si è verificato un errore permanente durante la valutazione di DMARC, ad esempio un record TXT DMARC formato erroneamente in DNS. Il tentativo di rinviare il messaggio non può terminare con un risultato diverso. Al contrario, potrebbe essere necessario contattare il proprietario del dominio per risolvere il problema.<br/>• **TempError**: si è verificato un errore temporaneo durante la valutazione di DMARC. Potrebbe essere possibile richiedere che il mittente invii il messaggio in un secondo momento per elaborare correttamente la posta elettronica.<br/>• **oreject** o o **. Reject**: è l'acronimo di rifiuto di sostituzione. In questo caso, Office 365 utilizza questa azione quando riceve un messaggio che non riesce a controllare DMARC da un dominio il cui record TXT DMARC ha un criterio di p = Reject. Invece di eliminare o rifiutare il messaggio, Office 365 contrassegna il messaggio come posta indesiderata. Per ulteriori informazioni sul motivo per cui Office 365 è configurato in questo modo, vedere [come office 365 gestisce la posta elettronica in ingresso che non riesce DMARC](use-dmarc-to-validate-email.md#inbounddmarcfail).<br/>• **PCT. Quarantine**: indica che una percentuale inferiore al 100% dei messaggi che non superano DMARC verrà recapitata comunque. Questo significa che il messaggio ha avuto esito negativo DMARC e che il criterio è stato impostato su Quarantine, ma il campo PCT non è stato impostato su 100% e il sistema ha determinato in modo casuale di non applicare l'azione DMARC in base al criterio del dominio specificato.<br/>• **PCT. Reject**: indica che una percentuale inferiore al 100% dei messaggi che non superano DMARC verrà recapitata comunque. Questo significa che il messaggio ha avuto esito negativo DMARC e che il criterio è stato impostato su Reject, ma il campo PCT non è stato impostato su 100% e il sistema ha determinato in modo casuale di non applicare l'azione DMARC in base al criterio del dominio specificato.|
