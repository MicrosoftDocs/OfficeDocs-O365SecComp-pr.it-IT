...
- È necessario utilizzare Exchange Online PowerShell per modificare la durata dell'attesa di conservazione per controversia legale in una cassetta postale inattiva. È possibile utilizzare l'interfaccia di amministrazione di Exchange (EAC). Ma è possibile utilizzare Exchange Online PowerShell o EAC per modificare la durata dell'attesa di an In-Place Hold. È possibile utilizzare la protezione di Office 365 &amp; centro conformità o sicurezza &amp; PowerShell centro conformità per modificare la durata dell'attesa di un criterio di conservazione di Office 365.
    
- Per connettersi a Exchange Online PowerShell o sicurezza &amp; PowerShell centro conformità, vedere uno degli argomenti seguenti:
...
|Mario Necaise  <br/> |Criterio di conservazione a livello di organizzazione Office 365 nella protezione &amp; centro conformità  <br/> |La proprietà *InPlaceHolds* è vuota. Indica che uno o più (livello di Exchange) o a livello di organizzazione Office 365 criterio di conservazione viene applicato alla cassetta postale inattiva. In questo caso, è possibile eseguire il "Get-OrganizationConfig | Select-Object - ExpandProperty InPlaceHolds` command in Exchange Online PowerShell to get a list of the GUIDs for organization-wide Office 365 retention policies. The GUID for organization-wide retention policies that are applied to Exchange mailboxes start with the  `mbx` prefix; for example  `mbxa3056bb15562480fadb46ce523ff7b02`.  <br/> <br/>To identity the Office 365 retention policy that's applied to the inactive mailbox, run the following command in Security &amp; Compliance Center PowerShell.  <br/><br/> `Get-RetentionCompliancePolicy<retention policy GUID without prefix> | Nome FL'<br/><br/>
|Carol Olson  <br/> |Criterio di conservazione di Office 365 nella protezione &amp; centro conformità applicate a determinate cassette postali  <br/> |La proprietà  *InPlaceHolds*  contiene il GUID del criterio di conservazione di Office 365 applicato alla cassetta postale inattiva. È possibile stabilire che si tratta di un criterio di conservazione applicato a specifiche cassette postali perché il GUID inizia con il prefisso  `mbx`. Tenere presente che il GUID del criterio di conservazione applicato alla cassetta postale inattiva iniziava con il prefisso  `skp`, indicando che il criterio di conservazione è applicato alle conversazioni di Skype for Business.  <br/><br/> Al criterio di conservazione identità Office 365 applicato alla cassetta postale inattiva, eseguire il comando seguente in sicurezza &amp; PowerShell centro conformità.<br/><br/> "Get-RetentionCompliancePolicy<retention policy GUID without prefix> | Nome FL` <br/><br/>Be sure to remove the  `mbx` or  `skp' quando si esegue questo comando.  <br/> |
|Abraham McMahon  <br/> |caso di eDiscovery attesa nella protezione &amp; centro conformità  <br/> |La proprietà  *InPlaceHolds*  contiene il GUID del blocco caso eDiscovery applicato alla cassetta postale inattiva. È possibile stabilire che si tratta di un blocco caso eDiscovery perché il GUID inizia con il prefisso  `UniH`.  <br/> È possibile utilizzare il `Get-CaseHoldPolicy` cmdlet in sicurezza &amp; PowerShell centro conformità per ottenere informazioni su eDiscovery caso in cui è associato il blocco per le cassette postali inattive. Ad esempio, è possibile eseguire il comando "Get-CaseHoldPolicy<hold GUID without prefix> | Nome FL` to display the name of the case hold that's on the inactive mailbox. Be sure to remove the  `UniH` prefix when you run this command.  <br/><br/> To identity the eDiscovery case that the hold on the inactive mailbox is associated with, run the following commands.  <br/><br/> `$CaseHold = Get-CaseHoldPolicy <hold GUID without prefix> `<br/><br/> `$CaseHold.CaseId Get-ComplianceCase | Nome FL'<br/><br/><br/> **Nota:** Si consiglia di non utilizzo eDiscovery contiene di cassette postali inattive. Ciò avviene perché casi di eDiscovery sono destinati ai casi specifici di specifici relativi a un problema legale. Un certo punto, sarà probabilmente terminare una controversia legale e vengono rimosse le conservazioni associate al caso e il caso di eDiscovery sarà chiuso (o eliminati). Infatti, se un'esenzione che viene inserita in una cassetta postale inattiva è associata a un caso eDiscovery e rilascia esenzione o viene chiuso o eliminato il caso di eDiscovery, cassette postali inattive verranno eliminate definitivamente. 

Per ulteriori informazioni sui criteri di conservazione di Office 365, vedere [Panoramica di criteri di conservazione](retention-policies.md).
...
3. Selezionare l'archiviazione sul posto da modificare e quindi fare clic su **Modifica** ![sull'icona Modifica](media/ebd260e4-3556-4fb0-b0bb-cc489773042c.gif).
    
4. Nella **In-Place eDiscovery &amp; attesa** proprietà pagina fare clic su **Archiviazione sul posto**. 
...
2. Fare clic su **specificare il numero di giorni di conservazione degli elementi rispetto al loro data di ricezione** per conservare gli elementi per un determinato periodo. Digitare il numero di giorni per cui si desidera conservare gli elementi per. 
