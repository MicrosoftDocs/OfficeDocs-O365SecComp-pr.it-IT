...
- [Attivazione di Azure RMS](#activate-azure-rms)
    
- [Configurazione di RMS in Exchange Online](#configure-rms-in-exchange-online)
    
- [Installazione del client Active Directory RMS](#install-the-active-directory-rms-client)
...
Mantenere il modulo di Azure Active Directory per Windows PowerShell aperto e connesso al servizio Microsoft online. Verrà eseguito un comando in questa finestra nel passaggio successivo.

## <a name="step-3-obtain-rms-tenant-id-and-licensing-url"></a>Passaggio 3: ottenere l'ID tenant RMS e l'URL delle licenze

Il passaggio successivo consiste nell'ottenere l'ID tenant e l'URL della posizione di gestione delle licenze per il servizio Azure RMS per l'organizzazione. Copiare e salvare queste informazioni nello stesso file che contiene la chiave simmetrica del passaggio 2. L'ID e l'URL verranno utilizzati nel passaggio 5 per crittografare i file PST.
...
7. Dopo aver visualizzato l'URL, copiarlo e salvarlo nel file in cui sono state salvate le altre chiavi. Assicurarsi di copiare l'intero URL. 
...
3. Eseguire il seguente comando per crittografare e caricare i file PST in Office 365.
    ...
    | `/protect-key:` <br/> |Specifica la chiave simmetrica ottenuta nel passaggio 2. Tenere presente che il valore di questo parametro è racchiuso tra virgolette doppie ("").  <br/> | `/protect-key:"l+R+Umc5RGmSBh1oW+DoyMxm/h5h2JJXFcNOFiNp867="` <br/> |
    | `/transfer:` <br/> |Specifica se si caricano i file PST sulla rete o li si inviano su un disco rigido. Il valore `upload` indica che il caricamento dei file viene eseguito tramite la rete. Il valore `drive` indica che si sta spedendo il PST su un disco rigido.<br/> | `/transfer:upload` <br/> |
...
6. Fare clic con il pulsante destro del mouse su **ingestiondata**e quindi scegliere **Apri editor contenitori BLOB**.
